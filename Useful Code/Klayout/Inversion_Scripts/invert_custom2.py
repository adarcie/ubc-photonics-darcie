...
def layout_invert_resist_tone(self):
  ...
  layer_high = layout.layer(pya.LayerInfo(1, 0))
  layer_low = layout.layer(pya.LayerInfo(1, 0))
  layer_in=layout.layer(pya.LayerInfo(100,0))
  ...
  if OUT_MODE == 2:
      ...
      pya.ShapeProcessor().size(layout,cell,layer_temp2,shapes_low,1010,pya.EdgeProcessor.ModeAnd,True,True,True)
      p.inc
  ...
  if OUT_MODE == 1:
      ...
      shapes_high = cell.shapes(layer_high)
      shapes_temp = cell.shapes(layer_temp)
      pya.ShapeProcessor().size(layout,topcell,layer_in,shapes_high,1999,pya.EdgeProcessor.ModeOr,True,True,True)
      p.inc
      pya.ShapeProcessor().size(layout,cell,layer_high, shapes_temp, 1,pya.EdgeProcessor.ModeAnd,True,True,True)
      p.inc
      shapes_high.clear()
