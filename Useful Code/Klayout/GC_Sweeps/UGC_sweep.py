...
def Example_mixed_waveguide_types():
  ...
  top_cell = cell
  cell = cell.layout().create_cell("UGC_sweep_cell")
#  t = Trans(Trans.R0, 0 / dbu, 0 / dbu)
  ...
  for row in range(num_rows):
    for col in range(num_cols):
      ...
      cell_taper = ly.create_cell("Universal Grating Coupler", "EBeam-dev", 
        {"theta_c":10, "wavelength":1.55*(1+row/100-(num_rows-1)/2/100) ,"Si_thickness":0.2,"etch_depth":0.2,"n_t":1,
        "dc":0.5*(1+3*col/100-3*(num_cols-1)/2/100)}
        ).cell_index()
    ...
    # Label for automated measurements, laser on Port 2, detectors on Ports 1, 3, 4
      ...
      text = pya.Text ("opt_in_TE_1550_device_UGC_"+str(row)+"_"+str(col), t)
