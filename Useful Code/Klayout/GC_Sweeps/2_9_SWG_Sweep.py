...
def Example_mixed_waveguide_types():
  ...
  pmper=0.05
  max_per=1.18939+pmper
  min_per=1.18939-pmper
...
#  t = Trans(Trans.R0, 0 / dbu, 0 / dbu)
  ...
  for row in range(num_rows):
    for col in range(num_cols):
      ...
      fillfac=0.244319
      dutycyc=0.488193
      gap=(perrow-fillfac*(perrow-dutycyc*perrow)-perrow*dutycyc)/2-errcol
      
      if gap>gaptol:
        # make grating coupler
        cell_taper = ly.create_cell("SWG Fibre Grating Coupler", "EBeam-dev", 
          { "wavelength":2.9 ,"n_e":3.1,"angle_e":20,
          "grating_length":32,"taper_length":32,"dc":dutycyc,"period":perrow,
          "ff":fillfac,"w_err":errcol}
          ).cell_index()
        ...
        dpath = DPath([DPoint(col*sepx+0,row*sepy+0), DPoint(col*sepx+10,row*sepy+0), DPoint(col*sepx+10,row*sepy+127),DPoint(col*sepx+0, row*sepy+127)], 1)
        cell.shapes(LayerSiN).insert(dpath.to_itype(dbu))
  
          
      # Label for automated measurements, laser on Port 2, detectors on Ports 1, 3, 4
        t = pya.Trans(Trans.R0,col*sepx/dbu, row*sepy/dbu)
        text = pya.Text ("opt_in_TE_2900_device_UGC_"+str(row)+"_"+str(col), t)
        shape = cell.shapes(TextLayerN).insert(text)
        shape.text_size = 10*dbu
      
  path_to_waveguide(cell = cell, verbose=True, params = 
    {'width': 1, 'adiabatic': True, 'radius': 10.0, 'bezier': 0.2, 
