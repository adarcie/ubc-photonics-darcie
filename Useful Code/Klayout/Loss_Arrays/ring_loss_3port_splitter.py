...
#  t = Trans(Trans.R0, 0 / dbu, 0 / dbu)
  ...
  for row in range(num_rows):#loop over rows (gap spacing)
    for col in range(num_cols):#loop over columns (ring radius)
      ...
      sepx=add_GC_offset+sepx1+y_length+10
      ...
      gcx=col*sepx-gc_xoffset-add_GC_offset+addx-y_length-10
      ...
      #insert y branch at input GC
      cell_y = ly.create_cell("ebeam_y_1550", "EBeam")
      t = Trans(Trans.R0, (gcx)/dbu+y_length/2/dbu, gcy/dbu+127/dbu)
      top_ring_cell.insert(CellInstArray(cell_y.cell_index(), t))
      
      
      
      #route top waveguide to pin1
      dpath = DPath([DPoint(gcx,gcy),DPoint((2*pin4x-pin1x)-10,gcy),DPoint((2*pin4x-pin1x)-10,gcy+127-y_branch_offset),DPoint(gcx+y_length,gcy+127-y_branch_offset)], 0.5)
      top_ring_cell.shapes(LayerSiN).insert(dpath.to_itype(dbu))
      
      #route bottom waveguide to pin3
      dpath = DPath([DPoint(gcx+y_length,gcy+127+y_branch_offset),DPoint((2*pin4x-pin1x),gcy+127+y_branch_offset),DPoint((2*pin4x-pin1x),pin3y)], 0.5)
      top_ring_cell.shapes(LayerSiN).insert(dpath.to_itype(dbu))
    ...
    # Label for automated measurements, laser on Port 2, detectors on Ports 1, 3, 4
      t = pya.Trans(Trans.R0,(col*sepx-gc_xoffset-add_GC_offset+addx-y_length-10)/dbu, (row*sepy+127)/dbu)
      text = pya.Text ("opt_in_TE_1550_device_ring_loss_3pt_r"+str(radius)+"_g"+str(gap), t)
