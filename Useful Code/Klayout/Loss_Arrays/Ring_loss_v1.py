...
#  t = Trans(Trans.R0, 0 / dbu, 0 / dbu)
  ...
  for row in range(num_rows):#loop over rows (gap spacing)
    for col in range(num_cols):#loop over columns (ring radius)
      ...
      t = Trans(Trans.R270, col*sepx/dbu, (row*sepy+spiral_y_offset)/dbu)
      top_ring_cell.insert(CellInstArray(cell_ring.cell_index(), t))
      t = Trans(Trans.R90, (col*sepx-(0.2-gap)*2-0.2)/dbu+w, (row*sepy+spiral_y_offset)/dbu)
      top_ring_cell.insert(CellInstArray(cell_ring.cell_index(), t))
      ...
      gcx=col*sepx-gc_xoffset-add_GC_offset
      gcy=row*sepy
      ...
      dpath = DPath([DPoint(gcx,gcy),DPoint(pin1x,gcy),DPoint(pin1x,pin1y)], 0.5)
      top_ring_cell.shapes(LayerSiN).insert(dpath.to_itype(dbu))
      
      #route bottom waveguide to pin3
      dpath = DPath([DPoint(gcx,gcy+127),DPoint(pin3x,gcy+127),DPoint(pin3x,pin3y)], 0.5)
      top_ring_cell.shapes(LayerSiN).insert(dpath.to_itype(dbu))
    ...
    # Label for automated measurements, laser on Port 2, detectors on Ports 1, 3, 4
      ...
      text = pya.Text ("opt_in_TE_1550_device_ring_loss_r"+str(radius)+"_g"+str(gap), t)
