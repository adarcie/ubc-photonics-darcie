...
def Example_mixed_waveguide_types():
  ...
  double_cols=False#for vertical space constraints, double up on columns in each row, splitting in half for different g
  num_rows=3
  num_cols=5
  ...
  GC_cell_name="1550_220_gc_JAN2021"
  WG_width=0.3#um
  ...
  top_cell = cell
  cell = cell.layout().create_cell("ring_sweep_cell_WG"+str(WG_width))
#  t = Trans(Trans.R0, 0 / dbu, 0 / dbu)
  ...
  for row in range(num_rows):#loop over rows (gap spacing)
    for col in range(num_cols):#loop over columns (ring radius)
      ...
      newcellname="ring_r"+str(radius)+"_g"+str(gap)+"_WG"+str(WG_width)
      ...
      cell_taper = ly.cell(GC_cell_name).cell_index()############################REPLACE
      t = Trans(Trans.R0, (gcx)/dbu, gcy/dbu)
      top_ring_cell.insert(CellInstArray(cell_taper, t))
      t = Trans(Trans.R0, (gcx)/dbu, gcy/dbu+127/dbu)
      top_ring_cell.insert(CellInstArray(cell_taper, t))
      cell_taper = ly.cell(GC_cell_name).cell_index()
      t = Trans(Trans.R0, (gcx)/dbu, gcy/dbu-127/dbu)
      top_ring_cell.insert(CellInstArray(cell_taper, t))
      t = Trans(Trans.R0, (gcx)/dbu, gcy/dbu+254/dbu)
      top_ring_cell.insert(CellInstArray(cell_taper, t))
      
      # if waveguide width<0.5, need to add taper
      if WG_width<0.5:
        cell_taper = ly.create_cell("ebeam_taper_te1550", "EBeam", { "wg_width1": 0.5, "wg_width2": WG_width, "wg_length":taper_length} )
        t = Trans(Trans.R0, (gcx)/dbu, gcy/dbu+127/dbu)
        top_ring_cell.insert(CellInstArray(cell_taper.cell_index(), t))
        t = Trans(Trans.R0, (gcx)/dbu, gcy/dbu-127/dbu)
        top_ring_cell.insert(CellInstArray(cell_taper.cell_index(), t))
        t = Trans(Trans.R0, (gcx)/dbu, gcy/dbu+254/dbu)
        top_ring_cell.insert(CellInstArray(cell_taper.cell_index(), t))
      ...
      dpath = DPath([DPoint(gcx+taper_length,gcy),DPoint((2*pin4x-pin1x),gcy),DPoint((2*pin4x-pin1x),pin1y)], WG_width)
      top_ring_cell.shapes(LayerSiN).insert(dpath.to_itype(dbu))
      
      #route bottom waveguide to pin3
      dpath = DPath([DPoint(gcx+taper_length,gcy+127),DPoint((2*pin4x-pin1x),gcy+127),DPoint((2*pin4x-pin1x),pin3y)], WG_width)
      top_ring_cell.shapes(LayerSiN).insert(dpath.to_itype(dbu))
    ...
    # Label for automated measurements, laser on Port 2, detectors on Ports 1, 3, 4
      ...
      text = pya.Text ("opt_in_TE_1550_device_ring_loss_4pt_r"+str(radius)+"_g"+str(gap)+"_WG"+str(WG_width), t)
